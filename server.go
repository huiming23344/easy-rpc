...
const (
	...
	defaultRPCPath   = "/_easy_rpc_"
	defaultDebugPath = "/debug/easy_rpc"
)

// ServeHTTP 实现了 http.Handler 可以回复 RPC 请求
...
// HandleHTTP 为在 rpcPath 上的 RPC 消息注册了一个 HTTP handler。
// 仍需要调用 http.Serve()，通常在 go 语句中。
...
// HandleHTTP 是 DefaultServer 注册 HTTP handler的快捷方法
func HandleHTTP() {
